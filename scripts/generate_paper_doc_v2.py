...
def add_run(para, text, bold=False, italic=False, size=11, color=None, font="Times New Roman"):
    ...
    run.italic = italic
    run.font.name = font
    # docx quirk: sometimes you need to set the eastern asian font and ascii font separately
...
heading(doc, "3.4 Phase 4: Physics-Informed Latent Regularization (Neural ODE)", 2)
body(doc, "The latent state Z evolves using a continuous-time Neural ODE regulated by a non-linear Jeffcott-Hertzian bearing model:")
equation(doc, "ℒ_phys = ||mẍ + cẋ + kx + k_h|x|^1.5 sgn(x) − F_ext||₁")

heading(doc, "3.5 Phase 5: Diagnostic Fusion and SI Scoring", 2)
body(doc, "The final Instability Score (SI) is computed as:")
equation(doc, "SI(t) = softmax( Σ wᵢ (f_i(t) − μ_h,i) / σ_h,i )")
...
add_table(doc,
    ["Metric", "Mean Value (10 Seeds)", "Variance (σ²)"],
    [
...
print("Saved docs/sangeeth1.paper1.doc")
